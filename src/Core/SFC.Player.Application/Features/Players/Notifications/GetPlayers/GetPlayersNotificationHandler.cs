...
namespace SFC.Player.Application.Features.Players.Notifications.GetPlayers;
public class GetPlayersNotificationHandler : INotificationHandler<GetPlayersEvent>
{
    private const int TOTAL = 10;
    private const int EXPIRATION_IN_MINUTES = 60;

    private readonly ILogger<GetPlayersEvent> _logger;
    ...
    public GetPlayersNotificationHandler(ILogger<GetPlayersEvent> logger, ICache cache, IOptions<CacheSettings> cacheConfig)
    {
        _logger = logger;
        _cache = cache;
    ...
    public async Task Handle(GetPlayersEvent notification, CancellationToken cancellationToken)
    {
