...
public static class GetPlayersByFiltersExtensions
{
    #region Public

    public static IEnumerable<Filter<Player>> BuildSearchFilters(this GetPlayersByFiltersFilterDto filter, DateTime now)
    {
        return new List<Filter<Player>> {
            new() {
    ...
    public static IEnumerable<Sorting<Player, dynamic>> BuildSearchSorting(this IEnumerable<SortingDto> sorting)
    {
        List<Sorting<Player, dynamic>> result = new();

        foreach (SortingDto sort in sorting)
        {
            Expression<Func<Player, dynamic>>? expression = sort.Name switch
            {
                ...
                nameof(GetPlayersByFiltersStatsFilterDto.Raiting) => p => p.Stats.Sum(m => m.Value),
                _ => null
            };
    ...
    public static Expression<Func<Player, bool>> FilterByStat(int? skill, short? from, short? to)
    {
        if (from.HasValue)
        {
    ...
    public static Expression<Func<Player, bool>> FilterByRaiting(int? raiting)
    {
